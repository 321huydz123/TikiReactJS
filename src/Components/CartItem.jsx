...
const CartItem = ({ item }) => {
    ...
    const { id, images, name, original_price, amount } = item;
    const thumbnailUrl = images.length > 0 ? images[0].thumbnail_url : null;
    const priceAmount = original_price * amount
    return (
        <section>
            <div className='grid grid-cols-8 gap-4 mt-10 items-center'>
                ...
                <Link to={`/product/${id}`} className='flex flex-col justify-center items-start col-span-2 '>
                    <div className='my-4 text-gray-500 hover:text-gray-900 hover:underline'>
                        {name}
                    </div>
                    <div className=" flex items-center justify-start">
                        <h2 className="text-xl font-bold">{formattedNumber(original_price)} đ</h2>
                    </div>
                    <div className=' relative '>
                        <h2 className='text-sm text-gray-500'>{formattedNumber(original_price)} đ</h2>
                ...
                <div>
                    <div className="flex justify-between items-center h-[40px] rounded-xl w-[120px] bg-transparent border-solid border-2 border-gray-200">
                        <div onClick={() => minusAmountItems(id)} className="p-2 cursor-pointer hover:text-red-500 text-gray-500">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 12h-15" />
                            </svg>
                        </div>
                        <div>{amount}</div>
                        <div onClick={() => plusAmountItems(id)} className="p-2 cursor-pointer hover:text-blue-500 text-gray-500">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <div className='col-span-2'>
                    <h2 className="text-xl items-center flex justify-center font-bold">{formattedNumber(priceAmount)} đ</h2>
                </div>
                <div onClick={() => removeFromCart(id)} className='flex cursor-pointer hover:text-gray-600 justify-center items-center'>
            ...
            <div className='h-[1px] mt-4 bg-gray-400 w-[90%] mx-auto'></div>
        </section >
    )
}
